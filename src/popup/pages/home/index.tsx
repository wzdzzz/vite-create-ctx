...
const Home = () => {
  ...
  const handleGetData = () => {
    if (chrome?.storage?.local) {
      chrome.storage.local.get(['list'], (result: { list: SetStateAction<EventItem[]>; }) => {
        if (result.list) {
          setList([...result.list])
        }
      })
  ...
  return <div className='home'>
    <div className="home-list">
      {
        list.map((item, index) => {
          return <div className='home-list-item' key={item.name}>
            ...
            <div className="home-list-item-btn">
              <div className="home-list-item-btn-delete" onClick={() => {
                handleDelete(index)
              }}>删除
              </div>
            </div>
    ...
    <div className='home-bottom'>
      <input type="text" placeholder='新增事件' value={params.name}
             onChange={(e) => setParams({...params, name: e.target.value})}/>
      <div className='home-bottom-btn' onClick={handleAdd}>新增</div>
    </div>
  </div>;
}
